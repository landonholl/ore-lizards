...
A Fabric mod for Minecraft 1.21.7 (Java 21, Mojang official mappings) that adds a single mob: the
Ore Lizard — a rare, invisible-while-dormant cave critter that erupts from the floor when a player
walks near, flees, then burrows back down. GeckoLib 5.2.1 drives its model/animations.

This branch (`1.21.7`) is the `1.21.8` port branch with the 1.21.7 builds of GeckoLib and Fabric API
...
the `## 1.2.0+mc1.21.7`, `## 1.2.0+mc1.21.8`, `## 1.2.0+mc1.21.5`, `## 1.2.0+mc1.21.4` and
...
- There is no `libs/` directory on this branch. The 1.20.1 original carries `libs/mclib-20.jar`
  ...
  GeckoLib 5.2.1 for 1.21.7 (like 5.2.2 for 1.21.8, 5.1.0 for 1.21.5, 4.8.5 for 1.21.4 and 4.9.2 for
...
- GeckoLib 5.x (5.1.0, 5.2.1 and 5.2.2 alike) logs one `@Mixin target
...
## 1.21.7-specific API notes

Things that bit during the port (1.21.1 first, then 1.21.4, 1.21.5 and 1.21.8 on top; 1.21.7 is the
...
- **1.21.6's `RenderPipeline` rework did not remove anything used here.** `RenderType.eyes`,
  ...
  direct `EntityRenderer.render(state, poseStack, bufferSource, light)` model on 1.21.7 and 1.21.8 —
  the submit/collector rework lands in 1.21.9, and *that* is where `OreTintLayer` will need real work.
- **GeckoLib 5.2.1 (this branch) and 5.2.2 (the `1.21.8` branch) are both drop-ins for 5.1.0 as far
  ...
  5.2.1 and 5.2.2. The one 5.2.1 → 5.2.2 difference in those classes is the loop shape of `GeoModel`'s
  animation lookup over `getAnimationResourceFallbacks`, which this mod (one animation file, no
  fallbacks) never notices. GeckoLib 5.2.1 declares `minecraft >=1.21.7` and is published on Modrinth
  for 1.21.7 and 1.21.8, so this branch's `fabric.mod.json` range is `>=1.21.7 <=1.21.8`; the `1.21.8`
  branch pins `1.21.8` exactly because 5.2.2 is published for that version alone.
...
- **`CompoundTag.getString`/`getBoolean` return `Optional`s**, `getStringOr`/`getBooleanOr` take a
  ...
  (see the 1.21.7 notes above) expose the same getter shapes, which is why the save/load bodies
